...
const stubs = {
  ...
  alignment: '({buildMaddTimingObservations(){return {observations:[{occurrence_id:occurrenceId,duration_ms:180,reference_harakah_ms:90,confidence:0.95,start_ms:100,end_ms:280,evidence:{}}],alignments:[{occurrence_id:occurrenceId,status:"aligned",start_ms:100,end_ms:280,duration_ms:180,confidence:0.95}]};}})',
...
assert.deepEqual(providerInput.referencePhonemes, ["a","a:","t"]);
assert.equal(providerInput.maddTargets.length, 1);
assert.equal(providerInput.maddTargets[0].occurrence_id, occurrenceId);
assert.deepEqual(
  {phoneme_start:providerInput.maddTargets[0].phoneme_start,phoneme_end:providerInput.maddTargets[0].phoneme_end},

...
export async function buildBurhanItqanBlueprint(input: { juz: number; testNumber: number; questionCount?: number }) {
  ...
  return {
    ...
    config: {
      ...
      min_recopy_ayahs: 5,
      max_recopy_ayahs: 30,
      variable_range_length: true,
      difficulty_progression: "test_number_1_to_10",
      cross_surah_ranges: true,
      short_surah_strategy: "variable_length_passage",
      question_mix: { recite_range: reciteCount, mutashabihat: mutashabihatCount, ayah_and_next: ayahNextCount },
      cumulative: false,

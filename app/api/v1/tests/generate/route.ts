...
const fragmentRecall = z.object({ type: z.literal("fragment_recall"), mode: z.enum(["word","sentence"]), surah_id: z.number().int().min(1).max(114), ayah_number: z.number().int().min(1), fragment: z.string().min(1).max(120), ayahs_after: z.number().int().min(0).max(5).optional() });
const question = z.discriminatedUnion("type", [
  z.object({ type: z.literal("mcq"), anchor: z.string().min(1).max(120).optional(), surah_id: z.number().int().min(1).max(114).optional(), ayah_number: z.number().int().min(1).optional() }).refine((value) => Boolean(value.anchor) || (value.surah_id != null && value.ayah_number != null), { message: "Provide anchor or surah_id + ayah_number." }),
...
const schema = z.object({ juz: z.number().int().min(1).max(30), level: z.number().int().min(1).max(7).default(1), test_type: z.enum(["non_cumulative","cumulative","custom"]).default("custom"), style: z.enum(["default","burhan_itqan"]).default("default"), test_number: z.number().int().min(1).max(10).optional(), persist: z.boolean().default(true), question_count: z.number().int().min(1).max(100).optional(), progression: z.enum(["from_30_to_1","from_1_to_30"]).default("from_30_to_1"), questions: z.array(question).min(1).max(100).optional() }).refine((value) => Boolean(value.questions?.length) || Boolean(value.question_count), { message: "Provide questions or question_count." });
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await generateTest({ juz: parsed.data.juz, level: parsed.data.level, testType: parsed.data.test_type, style: parsed.data.style, testNumber: parsed.data.test_number, progression: parsed.data.progression, persist: parsed.data.persist, questionCount: parsed.data.question_count, questions: parsed.data.questions }));
  } catch (error) {

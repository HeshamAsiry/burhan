...
const fragmentRecall = z.object({
  ...
  ayahs_after: z.number().int().min(0).max(5).optional(),
});
...
const question = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("anchor_recall"),
    anchor: z.string().min(1).max(120),
    occurrences_required: z
      .union([z.literal("all"), z.number().int().min(1).max(100)])
      .optional(),
    ayahs_after: z.number().int().min(0).max(10).optional(),
    juz_min: z.number().int().min(1).max(30).optional(),
    juz_max: z.number().int().min(1).max(30).optional(),
    include_surah: z.boolean().optional(),
  }),
  z.object({
    type: z.literal("mutashabihat"),
    anchor: z.string().min(1).max(120),
    occurrences_required: z
      .union([z.literal("all"), z.number().int().min(1).max(10)])
      .optional(),
    ayahs_after: z.number().int().min(0).max(5).optional(),
    ...
    generation_engine: z.enum(["default", "itqan_local"]).optional(),
  }),
  z.object({
    type: z.literal("recite_range"),
    start: endpoint,
    end: endpoint,
  }),
  fragmentRecall,
...
const schema = z
  .object({
    ...
    level: z.number().int().min(1).max(7).default(1),
    test_type: z
      .enum(["non_cumulative", "cumulative", "custom"])
      .default("custom"),
    style: z.enum(["default", "burhan_itqan"]).default("default"),
    test_number: z.number().int().min(1).max(10).optional(),
  ...
  .refine(
    ...
    { message: "Provide questions or question_count." },
  );
  ...
  try {
    ...
    return NextResponse.json(
      await generateTest({
        ...
        testNumber: parsed.data.test_number,
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error ? error.message : "Unable to generate test.",
      },
      { status: 500 },
    );
  }

...
const schema = z.object({ juz: z.number().int().min(1).max(30), level: z.number().int().min(1).max(7), test_type: z.string().min(1).max(50).default("custom"), persist: z.boolean().default(true), questions: z.array(question).min(1).max(100) });
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await generateTest({ juz: parsed.data.juz, level: parsed.data.level, testType: parsed.data.test_type, persist: parsed.data.persist, questions: parsed.data.questions }));
  } catch (error) {
